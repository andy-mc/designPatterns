  ...
  generate(size) {
    ...
    while (!isMagic) {
      ...
      for (let i = 0; i < size; ++i) {
        magicSquare.push(this.generator.generate(size));
      }

      const parsedMagicSquare = this.splitter.split(magicSquare);

      isMagic = this.verifier.verify(parsedMagicSquare);
    }
    return magicSquare;
  }
}

const ms = new MagicSquareGenerator();
const m = ms.generate(2);
console.log("m:", m);
